...
```
                  ...
                  (produce / fetch / group)
                        ...
                        ▼
               ┌────────────────┐
               │   TCP Server   │  (net.Listener, 1 goroutine per conn)
               │  connection.go │
               └───────┬────────┘
                        │  reads Kafka request frames
                        ▼
               ┌────────────────┐
               │    Handler     │  (handler.go → dispatch by ApiKey)
               │   Dispatch     │
               └──┬──────┬──┬──┘
                  │      │  │
       ┌──────────┘      │  └──────────────┐
       ▼                 ▼                  ▼
  ┌──────────┐   ┌──────────────┐   ┌──────────────┐
  │ Produce  │   │    Fetch     │   │  Group APIs  │
  │ Handler  │   │   Handler    │   │  (Join/Sync/ │
  │          │   │              │   │   Heartbeat) │
  └────┬─────┘   └──────┬───────┘   └──────┬───────┘
       │                │                   │
       └────────┬───────┘                   │
...
Application entry point. Parses CLI flags, loads YAML configuration, creates the storage engine (based on `storage.backend`), initialises the broker, starts the TCP server, and handles OS signals for graceful shutdown.
...
| File | Responsibility |
|------|---------------|
| `broker.go` | `Broker` struct – topic management, metadata, produce/fetch orchestration. Accepts a `StorageEngine` via dependency injection. |
...
| File | Responsibility |
|------|---------------|
| `config.go` | YAML configuration structs (`Config`, `BrokerConfig`, `StorageConfig`, etc.) and `Load()` / `Default()` |
...
3. Connection.Handle() loop:
   ...
   f. Handler writes response using protocol.Writer
   g. Connection writes 4-byte size prefix + response bytes
   h. Flush buffered writer to socket
   i. Return request buffer to pool
   j. Loop back to (a)
...
```
Client → Connection → handleProduce → Broker.Produce()
...
```
Client → Connection → handleFetch → Broker.Fetch()
